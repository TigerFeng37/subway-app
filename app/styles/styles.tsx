import StyleType from "../../types/Style";

const styles: Record<string, StyleType> = {
    'A': {
        letter: 'A',
        diamond: false,
        bgColor: 'bg-mta-blue',
        accentBgColor: 'bg-blue-100',
        darkAccentBgColor: 'bg-blue-200',
    },
    'C': {
        letter: 'C',
        diamond: false,
        bgColor: 'bg-mta-blue',
        accentBgColor: 'bg-blue-100',
        darkAccentBgColor: 'bg-blue-200',
    },
    'E': {
        letter: 'E',
        diamond: false,
        bgColor: 'bg-mta-blue',
        accentBgColor: 'bg-blue-100',
        darkAccentBgColor: 'bg-blue-200',
    },
    'B': {
        letter: 'B',
        diamond: false,
        bgColor: 'bg-mta-orange',
        accentBgColor: 'bg-orange-100',
        darkAccentBgColor: 'bg-orange-200',
    },
    'D': {
        letter: 'D',
        diamond: false,
        bgColor: 'bg-mta-orange',
        accentBgColor: 'bg-orange-100',
        darkAccentBgColor: 'bg-orange-200',
    },
    'F': {
        letter: 'F',
        diamond: false,
        bgColor: 'bg-mta-orange',
        accentBgColor: 'bg-orange-100',
        darkAccentBgColor: 'bg-orange-200',
    },
    'FX': {
      letter: 'F',
      diamond: true,
      bgColor: 'bg-mta-orange',
      accentBgColor: 'bg-orange-100',
      darkAccentBgColor: 'bg-orange-200',
    },
    'M': {
        letter: 'M',
        diamond: false,
        bgColor: 'bg-mta-orange',
        accentBgColor: 'bg-orange-100',
        darkAccentBgColor: 'bg-orange-200',
    },
    'G': {
        letter: 'G',
        diamond: false,
        bgColor: 'bg-mta-lime',
        accentBgColor: 'bg-g-accent',
        darkAccentBgColor: 'bg-g-dark-accent',
    },
    'L': {
        letter: 'L',
        diamond: false,
        bgColor: 'bg-mta-gray',
        accentBgColor: 'bg-slate-200',
        darkAccentBgColor: 'bg-slate-300',
    },
    'N': {
        letter: 'N',
        diamond: false,
        bgColor: 'bg-mta-yellow',
        accentBgColor: 'bg-yellow-accent',
        darkAccentBgColor: 'bg-yellow-200',
    },
    'Q': {
        letter: 'Q',
        diamond: false,
        bgColor: 'bg-mta-yellow',
        accentBgColor: 'bg-yellow-accent',
        darkAccentBgColor: 'bg-yellow-200',
    },
    'R': {
      letter: 'R',
      diamond: false,
      bgColor: 'bg-mta-yellow',
      accentBgColor: 'bg-yellow-accent',
      darkAccentBgColor: 'bg-yellow-200',
    },
    'W': {
      letter: 'W',
      diamond: false,
      bgColor: 'bg-mta-yellow',
      accentBgColor: 'bg-yellow-accent',
      darkAccentBgColor: 'bg-yellow-200',
    },
    'J': {
      letter: 'J',
      diamond: false,
      bgColor: 'bg-mta-brown',
      accentBgColor: 'bg-brown-accent',
      darkAccentBgColor: 'bg-brown-dark-accent',
    },
    'Z': {
      letter: 'Z',
      diamond: false,
      bgColor: 'bg-mta-brown',
      accentBgColor: 'bg-brown-accent',
      darkAccentBgColor: 'bg-brown-dark-accent',
    },
    '1': {
      letter: '1',
      diamond: false,
      bgColor: 'bg-mta-red',
      accentBgColor: 'bg-red-100',
      darkAccentBgColor: 'bg-red-200',
    },
    '2': {
      letter: '2',
      diamond: false,
      bgColor: 'bg-mta-red',
      accentBgColor: 'bg-red-100',
      darkAccentBgColor: 'bg-red-200',
    },
    '3': {
      letter: '3',
      diamond: false,
      bgColor: 'bg-mta-red',
      accentBgColor: 'bg-red-100',
      darkAccentBgColor: 'bg-red-200',
    },
    '4': {
      letter: '4',
      diamond: false,
      bgColor: 'bg-mta-green',
      accentBgColor: 'bg-green-accent',
      darkAccentBgColor: 'bg-dark-green',
    },
    '5': {
      letter: '5',
      diamond: false,
      bgColor: 'bg-mta-green',
      accentBgColor: 'bg-green-accent',
      darkAccentBgColor: 'bg-dark-green',
    },
    '6': {
      letter: '6',
      diamond: false,
      bgColor: 'bg-mta-green',
      accentBgColor: 'bg-green-accent',
      darkAccentBgColor: 'bg-dark-green',
    },
    '6X': {
      letter: '6',
      diamond: true,
      bgColor: 'bg-mta-green',
      accentBgColor: 'bg-green-accent',
      darkAccentBgColor: 'bg-dark-green',
    },
    '7': {
      letter: '7',
      diamond: false,
      bgColor: 'bg-mta-purple',
      accentBgColor: 'bg-purple-100',
      darkAccentBgColor: 'bg-purple-300',
    },
    '7X': {
      letter: '7',
      diamond: true,
      bgColor: 'bg-mta-purple',
      accentBgColor: 'bg-purple-100',
      darkAccentBgColor: 'bg-purple-300',
  },
    'FS': {
        letter: 'S',
        diamond: false,
        bgColor: 'bg-mta-dark-gray',
        accentBgColor: 'bg-gray-200',
        darkAccentBgColor: 'bg-gray-300',
    },
    'GS': {
        letter: 'S',
        diamond: false,
        bgColor: 'bg-mta-dark-gray',
        accentBgColor: 'bg-gray-200',
        darkAccentBgColor: 'bg-gray-300',
    },
    'H': {
        letter: 'S',
        diamond: false,
        bgColor: 'bg-mta-dark-gray',
        accentBgColor: 'bg-gray-200',
        darkAccentBgColor: 'bg-gray-300',
    },
    'SI': {
      letter: 'S',
      diamond: false,
      bgColor: 'bg-mta-blue',
      accentBgColor: 'bg-blue-100',
      darkAccentBgColor: 'bg-blue-200',
  },
  'SIR': {
    letter: 'S',
    diamond: false,
    bgColor: 'bg-mta-blue',
    accentBgColor: 'bg-blue-100',
    darkAccentBgColor: 'bg-blue-200',
},
}
export default styles;